...
_RUN_ID_RE = re.compile(r"^\d{8}-\d{6}-[0-9a-f]{16}$")


def test_format_matches_expected_pattern():
    run_id = generate_run_id()
    assert _RUN_ID_RE.match(run_id), f"run_id '{run_id}' doesn't match YYYYMMDD-HHMMSS-16hex"


def test_hex_suffix_is_16_chars():
    run_id = generate_run_id()
